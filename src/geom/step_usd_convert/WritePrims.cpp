...
void UsdStepExporter::writePrototypeXformsInPrototypesStage(
    ...
    for (int defIdx = 0; defIdx < total; defIdx++) {
        auto it = definitionNames.find(defs[defIdx].first);
        std::string rawName;
        if (it != definitionNames.end() && !isAutoGeneratedName(it->second) && !it->second.empty()) {
            rawName = it->second;
        } else {
            rawName = "Def_" + std::to_string(defIdx);
        }

        // Check for already-processed label touching the name counter,
        // so skipped entries don't consume a count slot
        if (prototypePaths.count(it->first)) {
            completed++;
            continue;
        }

        int protoCount = protoNameCounts[rawName]++;   // moved to after the early-out
