...
set_cache (${PROJ_NAME_UPPER}_REQUIRED_DEPS ""
           "Additional dependencies to consider required (semicolon-separated list, or ALL)")
set_cache (${PROJ_NAME_UPPER}_OPTIONAL_DEPS ""
           "Additional dependencies to consider optional (semicolon-separated list, or ALL)")
set_option (${PROJ_NAME_UPPER}_ALWAYS_PREFER_CONFIG
            "Prefer a dependency's exported config file if it's available" OFF)

set_cache (${PROJ_NAME_UPPER}_BUILD_MISSING_DEPS ""
     "Try to download and build any of these missing dependencies (or 'all' or 'required')")
set_cache (${PROJ_NAME_UPPER}_BUILD_LOCAL_DEPS ""
     "Force local builds of these dependencies if possible (or 'all')")

set_cache (${PROJ_NAME_UPPER}_LOCAL_DEPS_ROOT "${PROJECT_BINARY_DIR}/deps"
           "Directory were we do local builds of dependencies")
list (APPEND CMAKE_PREFIX_PATH ${${PROJ_NAME_UPPER}_LOCAL_DEPS_ROOT}/dist)
include_directories(BEFORE ${${PROJ_NAME_UPPER}_LOCAL_DEPS_ROOT}/dist/include)

# Build type for locally built dependencies. Default to the same build type
# as the current project.
set_cache (${PROJ_NAME_UPPER}_DEPENDENCY_BUILD_TYPE "${CMAKE_BUILD_TYPE}"
           "Build type for locally built dependencies")
set_option (${PROJ_NAME_UPPER}_DEPENDENCY_BUILD_VERBOSE
...
set_option (${PROJ_NAME_UPPER}_DEPENDENCY_BUILD_ALLOW_UNVERIFIED_TAGS
...
function (print_package_notfound_report)
    ...
    if (CFP_LOCALLY_BUILDABLE_DEPS_NOTFOUND OR CFP_LOCALLY_BUILDABLE_DEPS_BADVERSION)
        ...
        message (STATUS "    ${ColorBoldGreen}-D${PROJ_NAME_UPPER}_BUILD_MISSING_DEPS=required${ColorReset}")
        message (STATUS "  or")
        message (STATUS "    ${ColorBoldGreen}-D${PROJ_NAME_UPPER}_BUILD_MISSING_DEPS=all${ColorReset}")
    endif ()
    message (STATUS)
    message (STATUS "${ColorBoldYellow}=========================================================================${ColorReset}")
    message (STATUS)
...
#     global ${PROJ_NAME_UPPER}_BUILD_LOCAL_DEPS contains the package name or
#     is "all", it will behave as if set to "always", and if the variable
#     ${PROJ_NAME_UPPER}_BUILD_MISSING_DEPS contains the package name or is
...
macro (checked_find_package pkgname)
    ...
    if ("${pkgname}" IN_LIST ${PROJ_NAME_UPPER}_REQUIRED_DEPS
        OR "ALL" IN_LIST ${PROJ_NAME_UPPER}_REQUIRED_DEPS
        OR "all" IN_LIST ${PROJ_NAME_UPPER}_REQUIRED_DEPS)
        set (_pkg_REQUIRED 1)
    endif ()
    if ("${pkgname}" IN_LIST ${PROJ_NAME_UPPER}_OPTIONAL_DEPS
        OR "ALL" IN_LIST ${PROJ_NAME_UPPER}_OPTIONAL_DEPS
        OR "all" IN_LIST ${PROJ_NAME_UPPER}_OPTIONAL_DEPS)
        set (_pkg_REQUIRED 0)
        set (_pkg_OPTIONAL 1)
    endif ()
    # string (TOLOWER "${_pkg_BUILD_LOCAL}" _pkg_BUILD_LOCAL)
    if ("${pkgname}" IN_LIST ${PROJ_NAME_UPPER}_BUILD_LOCAL_DEPS
        OR ${PROJ_NAME_UPPER}_BUILD_LOCAL_DEPS STREQUAL "ALL"
        OR ${PROJ_NAME_UPPER}_BUILD_LOCAL_DEPS STREQUAL "all")
        set (_pkg_BUILD_LOCAL "always")
    elseif ("${pkgname}" IN_LIST ${PROJ_NAME_UPPER}_BUILD_MISSING_DEPS
            OR ${PROJ_NAME_UPPER}_BUILD_MISSING_DEPS STREQUAL "ALL"
            OR ${PROJ_NAME_UPPER}_BUILD_MISSING_DEPS STREQUAL "all"
            OR ("required" IN_LIST ${PROJ_NAME_UPPER}_BUILD_MISSING_DEPS
                AND _pkg_REQUIRED))
        set_if_not (_pkg_BUILD_LOCAL "missing")
    endif ()
    set (${pkgname}_local_build_script "${PROJECT_SOURCE_DIR}/cmake/build_deps/build_${pkgname}.cmake")
    ...
    # Version range is checked manually via reject_out_of_range_versions after
    # find_package returns. Passing it to find_package is unreliable for packages
    # that ship no ConfigVersion.cmake (e.g. pxr/OpenUSD).
    ...
    if (_enable OR (_pkg_REQUIRED AND NOT _pkg_OPTIONAL))
        ...
        elseif (_pkg_CONFIG OR _pkg_PREFER_CONFIG OR ${PROJ_NAME_UPPER}_ALWAYS_PREFER_CONFIG)
...
macro (build_dependency_with_cmake pkgname)
    cmake_parse_arguments(_pkg   # prefix
        ...
        "NOINSTALL"
        # singleValueKeywords:
        "GIT_REPOSITORY;GIT_TAG;GIT_COMMIT;VERSION;SOURCE_SUBDIR;VERBOSE"
        # multiValueKeywords:
        "CMAKE_ARGS"
        # argsToParse:
        ${ARGN})
    ...
    set (${pkgname}_LOCAL_SOURCE_DIR "${${PROJ_NAME_UPPER}_LOCAL_DEPS_ROOT}/${pkgname}")
    set (${pkgname}_LOCAL_BUILD_DIR "${${PROJ_NAME_UPPER}_LOCAL_DEPS_ROOT}/${pkgname}-build")
    set (${pkgname}_LOCAL_INSTALL_DIR "${${PROJ_NAME_UPPER}_LOCAL_DEPS_ROOT}/dist")
    ...
    if (NOT _pkg_VERBOSE OR "${_pkg_VERBOSE}" STREQUAL "")
    ...
    if (${PROJ_NAME_UPPER}_DEPENDENCY_BUILD_ALLOW_UNVERIFIED_TAGS
            AND NOT "${_pkg_GIT_TAG}" STREQUAL "")
        # Special case for CI bleeding edge test, which sets
        # ${PROJ_NAME_UPPER}_DEPENDENCY_BUILD_ALLOW_UNVERIFIED_TAGS to force
    ...
    if (${PROJ_NAME_UPPER}_DEPENDENCY_BUILD_VERBOSE)
    ...
    set(_pkg_CMAKE_ARGS ${_pkg_CMAKE_ARGS})
    ...
    execute_process (COMMAND
        ${CMAKE_COMMAND}
            ...
            # Same build type as us
                -DCMAKE_BUILD_TYPE=${${PROJ_NAME_UPPER}_DEPENDENCY_BUILD_TYPE}
            ...
            # Build args passed by caller
                ${_pkg_CMAKE_ARGS}
        ${_pkg_exec_quiet}
        RESULT_VARIABLE ${pkgname}_CONFIG_EXIT_CODE
        OUTPUT_VARIABLE ${pkgname}_CONFIG_OUTPUT
        ERROR_VARIABLE  ${pkgname}_CONFIG_OUTPUT
    )

    if(NOT ${${pkgname}_CONFIG_EXIT_CODE} EQUAL 0)
        message(FATAL_ERROR "[${pkgname}] Configuration failed (${${pkgname}_CONFIG_EXIT_CODE}):\n${${pkgname}_CONFIG_OUTPUT}")
    endif()

    # Build the package
    execute_process (COMMAND ${CMAKE_COMMAND}
                        --build ${${pkgname}_LOCAL_BUILD_DIR}
                        --config ${${PROJ_NAME_UPPER}_DEPENDENCY_BUILD_TYPE}
                     ${_pkg_exec_quiet}
                     RESULT_VARIABLE ${pkgname}_BUILD_EXIT_CODE
                     OUTPUT_VARIABLE ${pkgname}_BUILD_OUTPUT
                     ERROR_VARIABLE  ${pkgname}_BUILD_OUTPUT
    )

    if(NOT ${${pkgname}_BUILD_EXIT_CODE} EQUAL 0)
        message(FATAL_ERROR "[${pkgname}] Build failed (${${pkgname}_BUILD_EXIT_CODE}):\n${${pkgname}_BUILD_OUTPUT}")
    endif()
    ...
    if (NOT _pkg_NOINSTALL)
        execute_process (COMMAND ${CMAKE_COMMAND}
                            ...
                            --target install
                         ${_pkg_exec_quiet}
                         RESULT_VARIABLE ${pkgname}_INSTALL_EXIT_CODE
                         OUTPUT_VARIABLE ${pkgname}_INSTALL_OUTPUT
                         ERROR_VARIABLE  ${pkgname}_INSTALL_OUTPUT
                        )
        
        if(NOT ${${pkgname}_INSTALL_EXIT_CODE} EQUAL 0)
            message(FATAL_ERROR "[${pkgname}] Install failed (${${pkgname}_INSTALL_EXIT_CODE}):\n${${pkgname}_INSTALL_OUTPUT}")
        endif()
...
macro (install_local_dependency_libs pkgname libname)
    ...
    file (GLOB _lib_files
            ...
            "${${pkgname}_LOCAL_INSTALL_DIR}/lib/${${PROJ_NAME_UPPER}_DEPENDENCY_BUILD_TYPE}/*${libname}*"
         )
    ...
    if (WIN32)
        ...
        file (GLOB _lib_files
                ...
                "${${pkgname}_LOCAL_INSTALL_DIR}/bin/${${PROJ_NAME_UPPER}_DEPENDENCY_BUILD_TYPE}/*${libname}*.dll"
             )
...
# Compute the active Apple SDK version once per configure. This is used as a
# stand-in "version" for frameworks, which don't publish their own version
# the way find_package()-able libraries do.
if (NOT DEFINED _CHECKED_FRAMEWORK_SDK_VERSION)
    if (APPLE)
        execute_process (COMMAND xcrun --sdk macosx --show-sdk-version
                         OUTPUT_VARIABLE _CHECKED_FRAMEWORK_SDK_VERSION
                         OUTPUT_STRIP_TRAILING_WHITESPACE
                         ERROR_QUIET)
    endif ()
    set_if_not (_CHECKED_FRAMEWORK_SDK_VERSION "0.0")
endif ()


# checked_find_framework(pkgname ...) is a find_library()-based counterpart
# to checked_find_package(), for Apple frameworks (Metal, MetalKit,
# CoreImage, ...) that don't ship a *Config.cmake / Find*.cmake and so can
# never be located by find_package(). It mirrors checked_find_package's
# interface, enable/disable convention, and dependency-report bookkeeping as
# closely as possible so call sites and console output look the same:
#
#   checked_find_framework (Metal REQUIRED)
#   checked_find_framework (MetalKit REQUIRED DEPS Metal)
#
# On success it also creates an imported target `<pkgname>::<pkgname>`
# (e.g. Metal::Metal) so you can target_link_libraries() against it just
# like a real find_package() result, instead of a raw find_library() path.
#
# Supported options (subset of checked_find_package, adapted for frameworks):
#   * REQUIRED / OPTIONAL       - same meaning as checked_find_package.
#   * ENABLE <var>              - override the enabling variable name.
#   * DEPS <list>               - hard dependencies; disable if any unmet.
#   * ISDEPOF <downstream>      - only needed because <downstream> needs it.
#   * FRAMEWORK_NAME <name>     - actual framework name, if different from
#                                 pkgname (default: pkgname itself).
#   * DEFINITIONS <string>...   - passed to add_compile_definitions if found.
#   * SETVARIABLES <id>...      - variables to set TRUE if found.
#   * PRINT <list>              - extra variables to print in VERBOSE mode.
#   * DEBUG                     - extra debugging output.
#   * NO_RECORD_NOTFOUND        - don't record an unfound framework in the
#                                 dependency report.
#   * VERSION_MIN / VERSION_MAX - NOTE: frameworks don't expose a real
#                                 version, so this checks against the active
#                                 SDK version instead (see comment above).
#                                 Omit these if that proxy isn't meaningful
#                                 for what you're checking.
#   * RECOMMEND_MIN / RECOMMEND_MIN_REASON - same meaning as checked_find_package.
#
# N.B. Like checked_find_package, this must be a macro, not a function,
# because it needs to set variables that escape into the caller's scope.
macro (checked_find_framework pkgname)
    cmake_parse_arguments(_pkg   # prefix
        # noValueKeywords:
        "REQUIRED;OPTIONAL;DEBUG;NO_RECORD_NOTFOUND"
        # singleValueKeywords:
        "ENABLE;ISDEPOF;FRAMEWORK_NAME;VERSION_MIN;VERSION_MAX;RECOMMEND_MIN;RECOMMEND_MIN_REASON"
        # multiValueKeywords:
        "DEFINITIONS;PRINT;DEPS;SETVARIABLES"
        # argsToParse:
        ${ARGN})
    string (TOLOWER ${pkgname} pkgname_lower)
    string (TOUPPER ${pkgname} pkgname_upper)
    set_if_not (_pkg_FRAMEWORK_NAME ${pkgname})
    ...
    set_if_not (_pkg_VERSION_MIN "0.0")
    set_if_not (_pkg_VERSION_MAX "10000.0")

    #
    # Now actually go look for the framework
    #
    set (${pkgname}_FOUND FALSE)
    if (_enable OR (_pkg_REQUIRED AND NOT _pkg_OPTIONAL))
        if (NOT APPLE)
            set (${pkgname}_NOT_FOUND_EXPLANATION "(frameworks are only available on Apple platforms)")
        else ()
            find_library (${pkgname}_LIBRARY NAMES ${_pkg_FRAMEWORK_NAME})
            if (${pkgname}_LIBRARY)
                set (${pkgname}_FOUND TRUE)
                set_if_not (${pkgname}_VERSION ${_CHECKED_FRAMEWORK_SDK_VERSION})
            endif ()
        endif ()
        ...
        # Enforce the (SDK-version-based) VERSION_MIN / VERSION_MAX range,
        # the same way checked_find_package enforces it for real packages.
        if (${pkgname}_FOUND AND ${pkgname}_VERSION)
            reject_out_of_range_versions (${pkgname} ${${pkgname}_VERSION}
                                          ${_pkg_VERSION_MIN} ${_pkg_VERSION_MAX}
                                          _pkg_version_in_range)
            if (_pkg_version_in_range)
                ...
                message (STATUS "${ColorRed}${pkgname} (SDK ${${pkgname}_VERSION}) is outside the required range ${_pkg_VERSION_MIN}...${_pkg_VERSION_MAX} ${ColorReset}")
                list (APPEND CFP_ALL_BUILD_DEPS_BADVERSION ${pkgname})
            endif ()
        endif ()

        if (${pkgname}_FOUND)
            message (STATUS "${ColorGreen}Found ${pkgname} (framework, SDK ${${pkgname}_VERSION})${ColorReset}")
            ...
            set (${pkgname}_LIBRARIES ${${pkgname}_LIBRARY})
            ...
            if (_pkg_RECOMMEND_MIN)
                if (${${pkgname}_VERSION} VERSION_LESS ${_pkg_RECOMMEND_MIN})
                    message (STATUS "${ColorYellow}Recommend ${pkgname} SDK >= ${_pkg_RECOMMEND_MIN} ${_pkg_RECOMMEND_MIN_REASON} ${ColorReset}")
                endif ()
            endif ()
            string (STRIP "${pkgname} ${${pkgname}_VERSION}" app_)
            list (APPEND CFP_ALL_BUILD_DEPS_FOUND "${app_}")
        ...
        if (_pkg_VERBOSE AND (${pkgname}_FOUND OR _pkg_DEBUG))
            if (_pkg_DEBUG)
                dump_matching_variables (${pkgname})
            endif ()
            set (_vars_to_print ${pkgname}_LIBRARY ${pkgname}_LIBRARIES ${_pkg_PRINT})
            list (REMOVE_DUPLICATES _vars_to_print)
            foreach (_v IN LISTS _vars_to_print)
                if (NOT "${${_v}}" STREQUAL "")
                    message (STATUS "    ${_v} = ${${_v}}")
